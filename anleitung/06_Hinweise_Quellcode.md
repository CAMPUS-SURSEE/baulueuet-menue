...
- **Eine neue Spalte in `FELDER_KLASSE` muss in SharePoint existieren, bevor die Fassung live geht.** Beim *Lesen* ist das harmlos: Graph beantwortet ein unbekanntes Feld in `$select` mit HTTP 400, und `alleElemente` in `graph.js` wiederholt die Abfrage ohne Feldauswahl. Beim *Schreiben* gibt es diesen Rückfall nicht, das Speichern schlägt fehl. Gilt derzeit für `Teilnehmer` (erwartete Teilnehmeranzahl, Zahl, darf leer sein) und `Sprache` (Text `de`/`fr`/`en`, darf leer sein).
- **Die Sprache von Kursblatt und Gästeseite kommt aus dem Link.** Beide Seiten laden anonym über Flow B, der die Spalte `Sprache` nicht liefert. `admin.html` hängt über `Hilfe.spracheZusatz()` ein `&sprache=fr` an, das Kursblatt reicht es über `Hilfe.gastLinkMitSprache()` in den QR-Code weiter. Wer einen dieser Links von Hand zusammensetzt oder kürzt, bekommt Deutsch. Die Übersetzungen liegen je Seite in einem Objekt `TEXTE` mit den Schlüsseln `de`, `fr`, `en`; ein neuer Text gehört in alle drei. Die Werte, die an Flow C gehen (`Suppe`, `Salat`, `Keine`, `Menü 1`, `Menü 2`), bleiben in jeder Sprache gleich, übersetzt werden nur Beschriftungen.
