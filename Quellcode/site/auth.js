/* auth.js — Anmeldung an Entra ID (Microsoft 365) für die Admin-Seiten.
   ...
   Hier steht nur noch die Übersetzung in die vier Funktionen, welche die
   Seiten benutzen. Den OAuth-Ablauf selbst, das Zwischenspeichern der Token
   und die stille Erneuerung erledigt MSAL.
...
const Auth = (function () {
  ...
  const BEREICHE = ["https://graph.microsoft.com/Sites.ReadWrite.All"];

  let anwendungPromise = null;
  let konto = null;

  /* Die Umleitungsadresse muss exakt so in der App-Registrierung stehen.
     Ohne Abfragezeichenfolge, damit ?klasse=CODE nicht mitregistriert werden
     muss. MSAL merkt sich die vollständige Adresse selbst und kehrt am Ende
     dorthin zurück. */
  function zielUrl() {
