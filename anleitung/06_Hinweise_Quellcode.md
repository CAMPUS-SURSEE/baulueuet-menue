...
- **Die Sprache von Kursblatt und Gästeseite kommt aus dem Link, nicht aus SharePoint.** Am Termin wird keine Sprache gespeichert; die Spalte `Sprache` vom 08.09.2026 ist seit dem 09.09.2026 unbenutzt. `admin.html` nimmt den Wert aus dem Klappfeld `#kursblatt-sprache` im geteilten Knopf «Kursblatt drucken» und hängt über `Hilfe.spracheZusatz()` ein `&fr` oder `&en` an (blosser Schlüssel ohne Wert, damit der Link kurz bleibt; `spracheAusLink()` auf beiden Seiten versteht auch das ältere `&sprache=fr`); das Kursblatt reicht es über `Hilfe.gastLinkMitSprache()` in den QR-Code weiter. Wer einen dieser Links von Hand zusammensetzt oder kürzt, bekommt Deutsch. Auf der Gästeseite schalten die Knöpfe `.sprachwahl` über `spracheWechseln()` um: `spracheAnwenden()` setzt die feststehenden Texte, `ansichtNachzeichnen()` bildet alles neu, was aus Klassendaten oder Zustand entstanden ist; `zeigeFehler()` bekommt deshalb Textschlüssel statt Texte. Die Übersetzungen liegen je Seite in einem Objekt `TEXTE` mit den Schlüsseln `de`, `fr`, `en`; ein neuer Text gehört in alle drei. Die Werte, die an Flow C gehen (`Suppe`, `Salat`, `Keine`, `Menü 1`, `Menü 2`), bleiben in jeder Sprache gleich, übersetzt werden nur Beschriftungen.
- **`nachTagen()` in `admin.html` sortiert in eine Richtung, die der Sortierknopf umkehrt.** Der Zustand heisst `fruehesteOben`; Tage ohne Datum stehen in beiden Richtungen zuletzt. Eine Sonderregel für den heutigen Tag gehört nicht hinein, siehe den Punkt oben.
